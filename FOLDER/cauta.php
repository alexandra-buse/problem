...
<div class="container border_question p-3 my-3 bg-primary text-white">
  <p>
  <h2>Ai cautat: <?php echo $_GET['search']; ?><br> </h2> 
     <p style="font-size:100px">&#128540;</p> 
  <h1> Am gasit urmatoarele potriviri :  </h1>
...
$search = $_GET['search'];
$sql = "SELECT nume  , prenume, adresa  FROM utilizatori WHERE prenume='".$search."' OR nume='" .$search. "'" ;
$query = $db->query($sql);
if($query->num_rows > 0){
    while ($row = mysqli_fetch_assoc($query)){

		 echo "<p>".$row['nume']." ".$row['prenume']." -  ".$row['adresa']. "</p>";
        		

?>
<form action='alt_profil.php' method='get'>
